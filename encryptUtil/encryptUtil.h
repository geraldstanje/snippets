...
class EncryptUtil {
private:
	mutex lock; // handles multiple access to the stream_buffer, protects multiple write access
  vector<BYTE> key; // we store the key in memory
  vector<vector<BYTE>> thread_key; // each thread stores a copy of the key
...
private:
  ...
  void xor_encrypt_chunk(vector<BYTE> &stream_buffer, long first_proc_chunk, long number_of_bytes);
  void xor_encrypt_chunk(vector<BYTE> &stream_buffer, long first_proc_chunk, long number_of_bytes, long number_of_already_proc_bytes, long thread_id);
